...
lld-link: error: could not open &#39;oldnames.lib&#39;: No such file or directory
    ...
    <span class="token parameter variable">-target</span> x86_64-unknown-windows <span class="token punctuation">\\</span>
    -fuse-ld<span class="token operator">=</span>lld-link <span class="token punctuation">\\</span>
    <span class="token parameter variable">-nostdlib</span> <span class="token punctuation">\\</span>
    <span class="token parameter variable">-o</span> build/main.exe <span class="token punctuation">\\</span>
    build/main.o</span></span>
<span class="token output">lld-link: error: &lt;root&gt;: undefined symbol: mainCRTStartup
</span></code></pre><div class="highlight-lines"><br><br><br><div class="highlight-line"> </div><br><br><br></div><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>The linker is unable to find the C runtime entry point, <code>mainCRTStartup</code>, which makes sense because we&#39;re not linking the startup library. We can tell the linker to use our <code>main</code> function as the entry point by passing the <code>-entry:main</code> flag:</p><div class="language-sh-session line-numbers-mode" data-ext="sh-session"><pre class="language-sh-session"><code><span class="token command"><span class="token shell-symbol important">$</span> <span class="token bash language-bash">clang <span class="token punctuation">\\</span>
    <span class="token parameter variable">-target</span> x86_64-unknown-windows <span class="token punctuation">\\</span>
    -fuse-ld<span class="token operator">=</span>lld-link <span class="token punctuation">\\</span>
    <span class="token parameter variable">-nostdlib</span> <span class="token punctuation">\\</span>
    -Wl,-entry:main <span class="token punctuation">\\</span>
    <span class="token parameter variable">-o</span> build/main.exe <span class="token punctuation">\\</span>
    build/main.o</span></span>

<span class="token command"><span class="token shell-symbol important">$</span> <span class="token bash language-bash"><span class="token function">file</span> build/main.exe</span></span>
<span class="token output">build/main.exe: PE32+ executable (console) x86-64, for MS Windows
</span></code></pre><div class="highlight-lines"><br><br><br><br><div class="highlight-line"> </div><br><br><br><br><br></div><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Great! We have a PE32+ executable. But notice that it says <code>(console)</code>. This means that the executable is a console application, which cannot run on UEFI. We need to tell the linker to create a UEFI application instead by passing the <code>-subsystem:efi_application</code> flag:</p><div class="language-sh-session line-numbers-mode" data-ext="sh-session"><pre class="language-sh-session"><code><span class="token command"><span class="token shell-symbol important">$</span> <span class="token bash language-bash">clang <span class="token punctuation">\\</span>
    <span class="token parameter variable">-target</span> x86_64-unknown-windows <span class="token punctuation">\\</span>
    -fuse-ld<span class="token operator">=</span>lld-link <span class="token punctuation">\\</span>
    <span class="token parameter variable">-nostdlib</span> <span class="token punctuation">\\</span>
    -Wl,-entry:main <span class="token punctuation">\\</span>
    -Wl,-subsystem:efi_application <span class="token punctuation">\\</span>
    <span class="token parameter variable">-o</span> build/main.exe <span class="token punctuation">\\</span>
    build/main.o</span></span>

<span class="token command"><span class="token shell-symbol important">$</span> <span class="token bash language-bash"><span class="token function">file</span> build/main.exe</span></span>
<span class="token output">build/main.exe: PE32+ executable (EFI application) x86-64, for MS Windows
</span></code></pre><div class="highlight-lines"><br><br><br><br><br><div class="highlight-line"> </div><br><br><br><br><br></div><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Now we have a true UEFI application.</p><h2 id="cross-compiling-nim-to-pe32" tabindex="-1"><a class="header-anchor" href="#cross-compiling-nim-to-pe32" aria-hidden="true">#</a> Cross-compiling Nim to PE32+</h2><p>Let&#39;s try to do the same thing with Nim. We&#39;ll port the C program to Nim:</p><div class="language-nim line-numbers-mode" data-ext="nim"><pre class="language-nim"><code><span class="token comment"># main.nim</span>

<span class="token keyword">proc</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token operator">:</span> int <span class="token punctuation">{.</span>exportc<span class="token punctuation">.}</span> <span class="token operator">=</span>
    <span class="token keyword">return</span> <span class="token number">0</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>The <code>{.exportc.}</code> pragma tells the Nim compiler to export the function name as is, without any mangling. We do this because we need to pass the entry point name to the linker, and we don&#39;t want the compiler to mangle it.</p><p>Before we port this to Nim, we need to understand that Nim itself supports multiple targets. There are three arguments that influence the compilation/linking to a specific target:</p><ul><li><code>--cpu</code>(architecture), which defaults to the host architecture (in my case this is <code>amd64</code>, i.e. x86_64)</li><li><code>--os</code>(operating system), which defaults to the host operating system (in my case this is <code>linux</code>)</li><li><code>--cc</code>(backend compiler), which defaults to <code>gcc</code> (on Windows it relies on MinGW, which is a port of GCC to Windows)</li></ul><p>Nim does support cross-compiling to Windows using the <code>-d:mingw</code> flag. However, while the executable we want is a Windows executable format, the target OS is not Windows, but UEFI. Nim doesn&#39;t have a target OS for UEFI, so we&#39;ll need to use the <code>--os:any</code> flag to tell the compiler to not use any OS-specific code (it only expects a handful of ANSI C library functions to be available). We&#39;ll Nim to use specific values for these arguments.</p><p>We also pass the <code>clang</code> flags we used earlier to the compiler and linker using the <code>--passc</code> and <code>--passl</code> flags respectively.</p><div class="language-sh-session line-numbers-mode" data-ext="sh-session"><pre class="language-sh-session"><code><span class="token command"><span class="token shell-symbol important">$</span> <span class="token bash language-bash">nim c <span class="token punctuation">\\</span>
    <span class="token parameter variable">--nimcache:build</span> <span class="token punctuation">\\</span>
    <span class="token parameter variable">--cpu:amd64</span> <span class="token punctuation">\\</span>
    <span class="token parameter variable">--os:any</span> <span class="token punctuation">\\</span>
    <span class="token parameter variable">--cc:clang</span> <span class="token punctuation">\\</span>
    --passc:<span class="token string">&quot;-target x86_64-unknown-windows&quot;</span> <span class="token punctuation">\\</span>
    --passc:<span class="token string">&quot;-ffreestanding&quot;</span> <span class="token punctuation">\\</span>
    --passl:<span class="token string">&quot;-fuse-ld=lld-link&quot;</span> <span class="token punctuation">\\</span>
    --passl:<span class="token string">&quot;-nostdlib&quot;</span> <span class="token punctuation">\\</span>
    --passl:<span class="token string">&quot;-Wl,-entry:main&quot;</span> <span class="token punctuation">\\</span>
    --passl:<span class="token string">&quot;-Wl,-subsystem:efi_application&quot;</span> <span class="token punctuation">\\</span>
    --out:build/main.exe <span class="token punctuation">\\</span>
    main.nim</span></span>
<span class="token output">.../lib/system/osalloc.nim(218, 10) Error: Port memory manager to your platform
</span></code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>The compiler is complaining that it doesn&#39;t know how to allocate memory on this platform. This makes sense because we&#39;re not targeting any OS. Since we don&#39;t have an OS yet, we need a way to provide memory allocation primitives to the Nim compiler. The Nim docs say:</p><blockquote><p>The <code>-d:useMalloc</code> option configures Nim to use only the standard C memory manage primitives <code>malloc()</code>, <code>free()</code>, <code>realloc()</code>. If your platform does not provide these functions it should be trivial to provide an implementation for them and link these to your program.</p></blockquote><p>OK, at least we have a way to provide memory allocation primitives to Nim, instead of assuming they&#39;re provided by an existing OS (e.g. <code>mmap</code> on Linux or <code>VirtualAlloc</code> on Windows). Since we don&#39;t have an OS yet, let&#39;s implement a simple bump allocator backed by a fixed-size buffer. To keep things simple, we will not worry about freeing memory for now (we&#39;ll get to that later when we implement a proper memory manager).</p><div class="language-nim line-numbers-mode" data-ext="nim"><pre class="language-nim"><code><span class="token comment"># malloc.nim</span>

<span class="token punctuation">{.</span>used<span class="token punctuation">.}</span>

<span class="token keyword">var</span>
  heap<span class="token operator">*:</span> array<span class="token punctuation">[</span><span class="token number">1</span><span class="token operator">*</span><span class="token number">1024</span><span class="token operator">*</span><span class="token number">1024</span><span class="token punctuation">,</span> byte<span class="token punctuation">]</span> <span class="token comment"># 1 MiB heap</span>
  heapBumpPtr<span class="token operator">*:</span> int <span class="token operator">=</span> <span class="token function">cast[int]</span><span class="token punctuation">(</span><span class="token keyword">addr</span> heap<span class="token punctuation">)</span>
  heapMaxPtr<span class="token operator">*:</span> int <span class="token operator">=</span> <span class="token function">cast[int]</span><span class="token punctuation">(</span><span class="token keyword">addr</span> heap<span class="token punctuation">)</span> <span class="token operator">+</span> heap<span class="token operator">.</span>high

<span class="token keyword">proc</span> <span class="token function">malloc<span class="token operator">*</span></span><span class="token punctuation">(</span>size<span class="token operator">:</span> csize_t<span class="token punctuation">)</span><span class="token operator">:</span> pointer <span class="token punctuation">{.</span>exportc<span class="token punctuation">.}</span> <span class="token operator">=</span>
  <span class="token keyword">if</span> heapBumpPtr <span class="token operator">+</span> size<span class="token operator">.</span>int <span class="token operator">&gt;</span> heapMaxPtr<span class="token operator">:</span>
    <span class="token keyword">return</span> <span class="token keyword">nil</span>

  result <span class="token operator">=</span> <span class="token function">cast[pointer]</span><span class="token punctuation">(</span>heapBumpPtr<span class="token punctuation">)</span>
  inc heapBumpPtr<span class="token punctuation">,</span> size<span class="token operator">.</span>int

<span class="token keyword">proc</span> <span class="token function">calloc<span class="token operator">*</span></span><span class="token punctuation">(</span>num<span class="token operator">:</span> csize_t<span class="token punctuation">,</span> size<span class="token operator">:</span> csize_t<span class="token punctuation">)</span><span class="token operator">:</span> pointer <span class="token punctuation">{.</span>exportc<span class="token punctuation">.}</span> <span class="token operator">=</span>
  result <span class="token operator">=</span> <span class="token function">malloc</span><span class="token punctuation">(</span>size <span class="token operator">*</span> num<span class="token punctuation">)</span>

<span class="token keyword">proc</span> <span class="token function">realloc<span class="token operator">*</span></span><span class="token punctuation">(</span>p<span class="token operator">:</span> pointer<span class="token punctuation">,</span> new_size<span class="token operator">:</span> csize_t<span class="token punctuation">)</span><span class="token operator">:</span> pointer <span class="token punctuation">{.</span>exportc<span class="token punctuation">.}</span> <span class="token operator">=</span>
  result <span class="token operator">=</span> <span class="token function">malloc</span><span class="token punctuation">(</span>new_size<span class="token punctuation">)</span>
  <span class="token function">copyMem</span><span class="token punctuation">(</span>result<span class="token punctuation">,</span> p<span class="token punctuation">,</span> new_size<span class="token punctuation">)</span>
  <span class="token function">free</span><span class="token punctuation">(</span>p<span class="token punctuation">)</span>

<span class="token keyword">proc</span> <span class="token function">free<span class="token operator">*</span></span><span class="token punctuation">(</span>p<span class="token operator">:</span> pointer<span class="token punctuation">)</span> <span class="token punctuation">{.</span>exportc<span class="token punctuation">.}</span> <span class="token operator">=</span>
  <span class="token keyword">discard</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>Notice that I added the <code>{.used.}</code> pragma at the top of the file. This tells the compiler to consider the module as used, even if we don&#39;t call any of its procs directly. Otherwise, the compiler will consider it dead code and will eliminate it from the output.</p><p>For Nim to actually know about this module, we need to import it in our main module:</p><div class="language-nim line-numbers-mode" data-ext="nim"><pre class="language-nim"><code><span class="token comment"># main.nim</span>

<span class="token keyword">import</span> malloc
<span class="token operator">...</span>
    ...
    --passl:<span class="token string">&quot;-Wl,-entry:main&quot;</span> <span class="token punctuation">\\</span>
    --passl:<span class="token string">&quot;-Wl,-subsystem:efi_application&quot;</span> <span class="token punctuation">\\</span>
    <span class="token parameter variable">-d:useMalloc</span> <span class="token punctuation">\\</span>
    --out:build/main.exe <span class="token punctuation">\\</span>
    main.nim</span></span>
...
/home/khaled/.cache/nim/main_d/@mmain.nim.c:65:29: note: previous definition is here
   65 | N_LIB_PRIVATE N_NIMCALL(NI, main)(void) {
    ...
    --passc:<span class="token string">&quot;-target x86_64-unknown-windows&quot;</span> <span class="token punctuation">\\</span>
    --passc:<span class="token string">&quot;-ffreestanding&quot;</span> <span class="token punctuation">\\</span>
    --passc:<span class="token string">&quot;-I/usr/include&quot;</span> <span class="token punctuation">\\</span>
    --passl:<span class="token string">&quot;-target x86_64-unknown-windows&quot;</span> <span class="token punctuation">\\</span>
    --passl:<span class="token string">&quot;-fuse-ld=lld-link&quot;</span> <span class="token punctuation">\\</span>
    --passl:<span class="token string">&quot;-nostdlib&quot;</span> <span class="token punctuation">\\</span>
    --passl:<span class="token string">&quot;-Wl,-entry:main&quot;</span> <span class="token punctuation">\\</span>
    --passl:<span class="token string">&quot;-Wl,-subsystem:efi_application&quot;</span> <span class="token punctuation">\\</span>
...
<span class="token keyword">proc</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token operator">:</span> int <span class="token punctuation">{.</span>exportc<span class="token punctuation">.}</span> <span class="token operator">=</span>
    ...
    --passl:<span class="token string">&quot;-Wl,-subsystem:efi_application&quot;</span> <span class="token punctuation">\\</span>
    <span class="token parameter variable">-d:useMalloc</span> <span class="token punctuation">\\</span>
    <span class="token parameter variable">--noMain:on</span> <span class="token punctuation">\\</span>
    --out:build/main.exe <span class="token punctuation">\\</span>
    main.nim</span></span>
<span class="token output">...
...
</span></code></pre><div class="highlight-lines"><br><br><br><br><br><br><br><br><br><br><br><br><br><br><div class="highlight-line"> </div><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br></div><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>OK, the linker is complaining that it can&#39;t find some C functions. This is because we&#39;re targeting <code>--os:any</code>, which expects a handful of ANSI C library functions to be available for Nim to use:</p><ul><li><code>memset</code> and <code>memcpy</code> for some memory operations</li><li><code>strlen</code> for string length</li><li><code>fwrite</code> and <code>fflush</code> for writing to a file descriptor</li><li><code>stderr</code> for printing to standard error (not a function, but a global variable)</li><li><code>signal</code> for signal handlers</li><li><code>exit</code> for exiting the program</li></ul><p>Since our OS won&#39;t be a POSIX system, we can disable signals by passing the <code>-d:noSignalHandler</code> flag. For the rest of the functions, we&#39;ll need to implement them ourselves. Also, Nim includes implementation of some memory functions, which we can leverage by passing the <code>-d:nimNoLibc</code> flag.</p><p>Before we go any further, let&#39;s move the compiler flags to a <strong>nim.cfg</strong> file in the project root, so we don&#39;t have to pass them every time we compile:</p><div class="language-properties line-numbers-mode" data-ext="properties"><pre class="language-properties"><code><span class="token comment"># nim.cfg</span>

<span class="token key attr-name">--nimcache</span><span class="token punctuation">:</span><span class="token value attr-value">build</span>

<span class="token key attr-name">--noMain</span><span class="token punctuation">:</span><span class="token value attr-value">on</span>
<span class="token key attr-name">-d</span><span class="token punctuation">:</span><span class="token value attr-value">useMalloc</span>
<span class="token key attr-name">-d</span><span class="token punctuation">:</span><span class="token value attr-value">nimNoLibc</span>
<span class="token key attr-name">-d</span><span class="token punctuation">:</span><span class="token value attr-value">noSignalHandler</span>

<span class="token key attr-name">--cpu</span><span class="token punctuation">:</span><span class="token value attr-value">amd64</span>
<span class="token key attr-name">--os</span><span class="token punctuation">:</span><span class="token value attr-value">any</span>
<span class="token key attr-name">--cc</span><span class="token punctuation">:</span><span class="token value attr-value">clang</span>
...
<span class="token key attr-name">--passc</span><span class="token punctuation">:</span><span class="token value attr-value">&quot;-I/usr/include&quot;</span>
...
<span class="token key attr-name">--passl</span><span class="token punctuation">:</span><span class="token value attr-value">&quot;-Wl,-entry:main&quot;</span>
